...
# Int6 quantization: clip to [-31, 31] stored in int8, much better lzma compression
INT6_MAX = 31
...
def quantize_float_tensor(t: Tensor) -> tuple[Tensor, Tensor]:
    # Int6 quantization: clip to [-31, 31] stored in int8.
    # Lower range = lower entropy = much better lzma compression ratio.
    t32 = t.float()
    if t32.ndim == 2:
        clip_abs = (
...
class MLP(nn.Module):
    # LeakyReLU^2 MLP: preserves negative gradient flow, eliminates dead neurons
...
def main() -> None:
    ...
    # EMA model: shadow copy of base_model for evaluation
    ema_model: GPT | None = None
    if args.ema_enabled:
    ...
    while True:
        ...
        # EMA update: shadow model tracks base_model with exponential decay
    ...
    # Use EMA weights for serialization if available
    ...
    quant_obj, quant_stats = quantize_state_dict_int8(final_model.state_dict())
    quant_buf = io.BytesIO()
    torch.save(quant_obj, quant_buf)
    quant_raw = quant_buf.getvalue()
    # lzma gives significantly better compression than zlib on int6-range weights
    ...
    final_model.load_state_dict(dequantize_state_dict_int8(quant_state), strict=True)
    torch.cuda.synchronize()
