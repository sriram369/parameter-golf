...
# FA3 replaced with PyTorch SDPA (compile-compatible, uses FA2 kernel via enable_flash_sdp)
# FA3 requires flash_attn_interface (PyTorch 2.9+); SDPA works on any PyTorch 2.x
_ATTN_BACKEND = "sdpa"
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x: Tensor, q_w: Tensor, k_w: Tensor, v_w: Tensor, out_w: Tensor, v_embed: Tensor | None = None, v0: Tensor | None = None) -> tuple[Tensor, Tensor | None]:
        ...
        # Tiered attention: FA3 (best) -> FA2 -> PyTorch SDPA
        # FA3/FA2: [B, T, H, D] format natively (no transpose needed)
        # SDPA: needs [B, H, T, D] + GQA expand
        # PyTorch SDPA — compile-compatible, uses FA2 kernel via enable_flash_sdp(True)
        # q/k/v: [B, T, H, D]; SDPA needs [B, H, T, D] + head expansion for GQA
...
def main() -> None:
    ...
    log0(f"sdp_backends:cudnn=False flash=True mem_efficient=False math=False attn_backend:{_ATTN_BACKEND}")
